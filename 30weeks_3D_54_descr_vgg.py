...
def load_descriptors(imagepaths):
    number=[]
    for i in xrange(len(imagepaths)):
        image=os.path.splitext(os.path.basename(imagepaths[i]))[0]
        number.append(int(image[:5]))
    df = pandas.read_excel('/home/joao/Code/30wks/30weeks_descriptors.xlsx')
    meancurvature,gyrificationindex,medianthickness,outersurface,innersurface=[],[],[],[],[]
    for d in xrange(len(number)):
        innersurface.append(np.float64(df[df.patientid == number[d]]['innersurface']))
        outersurface.append(float(df[df.patientid == number[d]]['outersurface']))
        medianthickness.append(float(df[df.patientid == number[d]]['medianthickness']))
        gyrificationindex.append(float(df[df.patientid == number[d]]['gyrificationindex']))
        meancurvature.append(float(df[df.patientid == number[d]]['meancurvature']))
    return innersurface, outersurface,medianthickness, gyrificationindex, meancurvature

train_innersurface, train_outersurface,train_medianthickness, train_gyrificationindex, train_meancurvature=load_descriptors(trainimagepaths)
val_innersurface, val_outersurface,val_medianthickness, val_gyrificationindex, val_meancurvature=load_descriptors(valimagepaths)
#test_innersurface, test_outersurface,test_medianthickness, test_gyrificationindex, test_meancurvature=load_descriptors(testimagepaths)
...
def getmodel(X1,p_drop_hidden, height, width, deepness):             
    convnetwork = lasagne.layers.InputLayer(shape=(None, 1,deepness, height, width),input_var=X1)
    
    for i in xrange(3): 
        convnetwork = lasagne.layers.Conv3DLayer(convnetwork, num_filters=8, filter_size=(3, 3, 3),nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.GlorotUniform())        
    print convnetwork.output_shape
        
    convnetwork = lasagne.layers.MaxPool3DLayer(convnetwork,pool_size=(1,2,2)) 
    print convnetwork.output_shape
    ...
    network = lasagne.layers.DenseLayer(lasagne.layers.dropout(network, p=p_drop_hidden),num_units=13,nonlinearity=lasagne.nonlinearities.linear)     
...
model, convmodel = getmodel(X1, 0.5, height, width, deepness)
...
def DescrLabel(innersurface, outersurface,medianthickness, gyrificationindex, meancurvature):
    descriptors=np.concatenate(([innersurface], [outersurface],[medianthickness], [gyrificationindex], [meancurvature]))
    all_descriptors=np.swapaxes(descriptors,0,1)
    return all_descriptors

new_train_descriptors=DescrLabel(train_innersurface, train_outersurface,train_medianthickness, train_gyrificationindex, train_meancurvature)
new_val_descriptors=DescrLabel(val_innersurface, val_outersurface,val_medianthickness, val_gyrificationindex, val_meancurvature)

train_tutti=np.concatenate((train_sum_labels,new_train_descriptors),axis=1)
val_tutti=np.concatenate((val_sum_labels,new_val_descriptors),axis=1)
...
def normalization(labels,trainlabels):
    ...
    reshaped_np_normalized=np.reshape(np_normalized,(13,labels.shape[0]))
    ordered_np_normalized=np.swapaxes(reshaped_np_normalized,0,1)
    return(ordered_np_normalized)
new_ttt_volumes=normalization(train_tutti,train_tutti)
new_vvv_volumes=normalization(val_tutti,train_tutti)
...
def WholeTransformation(batch_images, batch_volumes, number_of_augmentations): 
    transform_matrix=[]
    all_volumes=[]
    for k in (xrange(batch_images.shape[0])):
        for j in (xrange(number_of_augmentations)):
            transf_chunk=[]
            scale_factor=random.uniform(1, 1)
            angle=randint(-15,15)
            tx=randint(0,0)
            ty=randint(0,0)
            intensity_number=np.random.uniform(0.87, 1.13)
...
for i in xrange(minibatches):
    ...
    t_volumes=new_ttt_volumes[imagesample]
    tt0 = time.time()
    full_t_images,full_t_volumes=WholeTransformation(t_images,t_volumes,10)
    tt1 = time.time()
    ttt0 = time.time()
    ...
    v_volumes=new_vvv_volumes[validimagesample]
    full_v_images,full_v_volumes=WholeTransformation(v_images,v_volumes,10)
    y=validation(full_v_images,full_v_volumes)
    validlosslist.append(y)
    if  i==2000 or i==4000 or i==6000 or i==8000 or i==10000 or i==12000 or i==14000 or i==16000 or i==18000 or i==20000 or i==22000 or i==24000:
        print("Mais 2000")

t1 = time.time()
print 'Training time: {} seconds'.format(t1-t0)
print 'Augmentation time: {} seconds'.format(tt1-tt0)
print 'Actual training time: {} seconds'.format(ttt1-ttt0)

plt.figure()
plt.plot(losslist)
plt.plot(validlosslist)
plt.yscale("log")
plt.savefig('3D_full_validlosslist')
plt.figure()
plt.plot(losslist)
plt.yscale("log")
plt.savefig('3D_full_trainloss')
...
save_weights('/home/joao/Code/ModelsVolumes/[3D_FULL]model_3Dconvs_25000_42minibat_8_filt.pkl', model)


validsamples = np.arange(len(array_val))
validation_sample=random.sample(validsamples,13)
validation_images=convert_to_np(array_val, validation_sample)
validation_volumes=new_vvv_volumes[validation_sample]
print(validation_volumes.shape)
print(validation_images.shape)
augmented_val_images,augmented_val_volumes=WholeTransformation(validation_images, validation_volumes, 20)
...
save_volumes('/home/joao/Code/ModelsVolumes/[3D_FULL]volumes_3Dconvs_25000_42minibat_8_filt.pkl', predicted_results)
...
index = {0: 'CB', 1: 'mWM', 2: 'BGT', 3: 'vCSF', 4: 'uWM', 5: 'BS', 6: 'GM', 7: 'eCSF', 
         8: 'InnerSurface', 9: 'OuterSurface', 10: 'MedianThickness', 11: 'GyrificationIndex', 12: 'MeanCurvature'}
correlation, correlation_r2, mse=[],[],[]
for r in xrange(13):
    plt.figure()
    plt.plot(predicted_results[:,r:r+1], labeled_results[:,r:r+1], '*')
    plt.plot(predicted_results[:13,r:r+1], labeled_results[:13,r:r+1], 'r*')
    plt.xlabel('Test Results')
    plt.ylabel('Real Results')
    plt.plot([0,1],[0,1])
    plt.savefig('3D_full_graph_'+index[r])
    plt.figure()
    ...
    plt.axhline(mean_all - 1.96*std, color='red', linestyle='--')
    plt.savefig('3D_full_graph_Bland-Aldman'+index[r])
...
print(correlation[0][0][1], correlation[1][0][1], correlation[2][0][1], correlation[3][0][1], 
      ...
      correlation[8][0][1], correlation[9][0][1], correlation[10][0][1], correlation[11][0][1],
      correlation[12][0][1])
...
print(correlation[0][0][1]**2, correlation[1][0][1]**2, correlation[2][0][1]**2, correlation[3][0][1]**2, 
      ...
      correlation[8][0][1]**2, correlation[9][0][1]**2, correlation[10][0][1]**2, correlation[11][0][1]**2,
      correlation[12][0][1]**2)
...
print(correlation_r2[0], correlation_r2[1], correlation_r2[2], correlation_r2[3], 
      ...
      correlation_r2[8], correlation_r2[9], correlation_r2[10], correlation_r2[11],
      correlation_r2[12])

print("MSE for CB, mWM, BGT, vCSF, uWM, BS, GM, eCSF:")
print(mse[0], mse[1], mse[2], mse[3], mse[4], mse[5], mse[6], mse[7], 
      mse[8], mse[9], mse[10], mse[11], mse[12])
...
print(sqrt(mse[0]), sqrt(mse[1]), sqrt(mse[2]), sqrt(mse[3]), 
      ...
      sqrt(mse[8]), sqrt(mse[9]), sqrt(mse[10]), sqrt(mse[11]),
      sqrt(mse[12]))
