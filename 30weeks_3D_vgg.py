...
def getmodel(X1,p_drop_hidden, height, width, deepness):             
    ...
    for i in xrange(3): 
        convnetwork = lasagne.layers.Conv3DLayer(convnetwork, num_filters=8, filter_size=(2, 3, 3),nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.GlorotUniform())        
    print convnetwork.output_shape
        
    convnetwork = lasagne.layers.MaxPool3DLayer(convnetwork,pool_size=(2,2,2)) 
    print convnetwork.output_shape
    
    for i in xrange(3): 
        convnetwork = lasagne.layers.Conv3DLayer(convnetwork, num_filters=8, filter_size=(1, 3, 3),nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.GlorotUniform())    
    print convnetwork.output_shape
